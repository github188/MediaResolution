...
#include "LogSys.h"
	...
	#endif
#endif
#pragma comment(lib,"./VDHaiKangDVR/HCNetSDK.lib")
...
class VDHaiKangDVR : public VirtualDevice
{
public:
	VDHaiKangDVR();
	~VDHaiKangDVR();
...
public:
	...
	*/
	...
	//查询录像
	//rd: 参考Record说明
	bool QueryRecord(long lChannel, const LocalDateTime& tStart, const LocalDateTime& tEnd,vector<Record>& rds);
...
private:
	...
	{
		if ( NET_DVR_SetDVRConfig(m_lUserID,NET_DVR_SET_COMPRESSCFG_V30,lChannel,
			&video_Para_records[lChannel].m_dvr_compress_cfg,sizeof(NET_DVR_COMPRESSIONCFG_V30)) )
			return true;

		m_Config.vp_log->log(Message::PRIO_NOTICE,"设置压缩参数失败, Channel = %ld, Error = %lu", lChannel, NET_DVR_GetLastError());
		return false; 
	}
	bool SetPicturInfo(long lChannel)
	{
		if ( NET_DVR_SetDVRConfig(m_lUserID,NET_DVR_SET_PICCFG_V30,lChannel,
	...
	bool SetExPicturInfo(long lChannel)
	{
		...
		struVideoPara.dwChannel=lChannel;
		DWORD DvrVal[4]={0};
	...
	{
		if ( NET_DVR_SetDVRConfig(m_lUserID,NET_DVR_SET_DECODERCFG_V30,lChannel,
...
private:
	...
	BYTE m_byChannelCount;          //模拟通道个数
};

//提供对象工厂
class VDHaiKangDVRMaker : public VirtualDeviceMaker
{
public:
	VDHaiKangDVRMaker();
	~VDHaiKangDVRMaker();
