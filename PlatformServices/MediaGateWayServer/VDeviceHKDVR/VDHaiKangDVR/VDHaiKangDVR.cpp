#include "stdafx.h"
#include "VDHaiKangDVR.h"
#include "Markup.h"

bool g_bInitSDK = false;
hash_map<string, VDHaiKangDVR*> g_VPHaiKang;

int g_sequence=0;                         //add by sunminping
									 ...
									 DWORD dwBufLen, void* pUser)
{
	try
	{
		string sIP = (char*)pUser;
		VDHaiKangDVR * p;
		{
			Poco::FastMutex::ScopedLock locker(g_sdkLock);
			hash_map<string, VDHaiKangDVR*>::iterator pos;
			pos = g_VPHaiKang.find(sIP);

			if ( sIP.empty() || pos == g_VPHaiKang.end() )
			{
				return;
			}
...
bool VDHaiKangDVR::InitPTZCommand()
{
	DWORD dwStop = 1<<16;
...
bool VDHaiKangDVR::InitPicCfg(long lChannel)
{
	DWORD dwReturned = 0;
	if(!NET_DVR_GetDVRConfig(m_lUserID, NET_DVR_GET_PICCFG_V30, lChannel, &video_Para_records[lChannel].m_dvr_pic_cfg, 
	...
	if(!NET_DVR_GetDVRConfig(m_lUserID, NET_DVR_GET_PICCFG_EX, lChannel, &video_Para_records[lChannel].m_dvr_pic_cfg_ex, 
...
bool VDHaiKangDVR::InitCompressCfg(long lChannel)
{
	DWORD dwReturned = 0;
...
VDHaiKangDVR::VDHaiKangDVR()
{
    m_sVersion = "HaiKang";//VPHaiKang_Version;
	m_dwLastCommand=0;
    m_lUserID = -1;
										...
										BYTE byAudioFlag, DWORD dwUser)
{
	...
	VDHaiKangDVR * p = (VDHaiKangDVR*)dwUser;
	if ( byAudioFlag == 1 )
	{
...
BOOL VDHaiKangDVR::MessageNotify(LONG lCommand, char *pBuf,DWORD dwBufLen)
{
	try
	{
...
void CALLBACK RealDataCallBack(LONG lRealHandle, DWORD dwDataType, BYTE *pBuffer,DWORD dwBufSize,void* dwUser)
{
	try
	{
        ...
        VDHaiKangDVR* pPlatform = (VDHaiKangDVR*)pChannel->vplatform_;
		...
		if (dwDataType == NET_DVR_SYSHEAD && pChannel->is_send_videoheader_==false)
		{
			...
			pPlatform->m_pCallBack->NotifyStreamData(pChannel->channelno_,lType,head_frame, (PBYTE)head_str.c_str(), head_str.size(), pChannel->distribution_manager_);
...
int VDHaiKangDVR::Login(VDeviceConfig& dev_config)
{
	...
	//m_byChannelCount=m_dvr_info.byChanNum;

	if(m_lUserID < 0)
	{
		//登录失败
		DWORD dwError = NET_DVR_GetLastError();  
		m_Config.vp_log->log(Message::PRIO_ERROR, "HaiKangDVR device id %s NET_DVR_Login_V30失败!", m_Config.deviceID);
...
//初始化
int VDHaiKangDVR::Init(VDeviceConfig& dev_config) 
{
	g_sdkLock.lock();
	if (!g_bInitSDK)
	{
		if(!NET_DVR_Init())
		{
			m_Config.vp_log->log(Message::PRIO_ERROR, "HaiKangDVR NET_DVR_Init失败!");
            g_sdkLock.unlock();
			return false;
		}
		g_bInitSDK=true;
...
void VDHaiKangDVR::RegisterObject()
{
	...
	_key.assign(buf);
	g_VPHaiKang[ _key ] = this;
}

void VDHaiKangDVR::UnRegisterObject()
{
	Poco::ScopedLock<Poco::FastMutex> lock(g_sdkLock);
	g_VPHaiKang.erase(_key );
}

bool VDHaiKangDVR::SetupAlarmChannel()
{
	//SDK中收到DVR发过来的信息需要上传的通过回调函数实现，比如报警信息，日志信息等。
	NET_DVR_SetDVRMessageCallBack_V30(MessageCallBack, (void*)_key.c_str());
...
void VDHaiKangDVR::CloseAlarmChannel()
{
...
bool VDHaiKangDVR::SetAlarmState(long lChannel, long lType, long lState) 
{
	//HxTrace("VDHaiKangDVR::SetAlarmState Channe:%d Type:%d State:%d",lChannel,lType,lState);
	bool bSuccess = false;
	switch (lType)
	{
	case alarm_camera:
	...
	case alarm_out:
		{
			if (NET_DVR_SetAlarmOut(m_lUserID,lChannel,lState))
				...
				m_Config.vp_log->log(Message::PRIO_ERROR, "HaiKangDVR device id %s NET_DVR_SetAlarmOut失败!", m_Config.deviceID);
		}
		break;
	default:
		break;
...
bool VDHaiKangDVR::GetAlarmState(long lChannel, long lType, long& lState) 
{
	bool bSuccess = false;
	...
	case alarm_out:
		{
			...
			{
				m_Config.vp_log->log(Message::PRIO_ERROR, "HaiKangDVR device id %s NET_DVR_GetAlarmOut_V30失败!", m_Config.deviceID);
				break;
			}
...
//设置时间OSD
bool VDHaiKangDVR::SetTime(Time& t) 
{ 
	NET_DVR_TIME dt;
...
//实时视频
bool VDHaiKangDVR::PlayVideo(long lChannel,void* dm, bool bMainStrean)
{   
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	...
	if (lHandle < 0)
	{
		m_Config.vp_log->log(Message::PRIO_ERROR, "HaiKangDVR device id %s NET_DVR_RealPlay_V40 fail,errno=%lu", m_Config.deviceID,NET_DVR_GetLastError());
		return false;
	}
...
bool VDHaiKangDVR::StopVideo(long lChannel,bool bMainStrean)
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	long lPlayChannelId = GetChannelPlayId(lChannel,bMainStrean);
...
//////////////////////////////////////////////////////////////////////////
bool VDHaiKangDVR::QueryRecord(long lChannel, const LocalDateTime& tStart, const LocalDateTime& tEnd,vector<Record>& rds)
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	//兼容板卡
	...
	if(lFind == -1)
	{
		m_Config.vp_log->log(Message::PRIO_ERROR, "HaiKangDVR device id %s NET_DVR_FindFile_V30 fail", m_Config.deviceID);
...
bool VDHaiKangDVR::RecordRequest(long lChannel,long lRequestType, Record& rd, long& lContext,long& lData, int& down_pos)
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if ( lRequestType == 0 )
	...
	if ( lRequestType == 1 )
	{
		...
		if(lContext == -1)
		{
			m_Config.vp_log->log(Message::PRIO_ERROR, "HaiKangDVR device id %s NET_DVR_GetFileByTime fail", m_Config.deviceID);
			return false;
		}

		if ( !NET_DVR_PlayBackControl(lContext, NET_DVR_PLAYSTART, 0, NULL) )
		{
			m_Config.vp_log->log(Message::PRIO_ERROR, "HaiKangDVR device id %s NET_DVR_PlayBackControl fail", m_Config.deviceID);
			return false;
		}
	}
...
bool VDHaiKangDVR::GetNetParam(string& sIPAddr, string& sNetMask, string& sGateway)
{
	DWORD dwReturned = 0;
	...
	{
		m_Config.vp_log->log(Message::PRIO_ERROR, "HaiKangDVR device id %s NET_DVR_GetDVRConfig 获取网络参数失败!", m_Config.deviceID);
		return false;
	}
...
//云台控制
bool VDHaiKangDVR::PTZControl(long lChannel,long lCommand, long lValue, void* pParam)
{ 
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
...
void VDHaiKangDVR::GetPointForPlace(WORD& x, WORD& y, WORD lPlace)
{
	//区域大小704*576
...
void VDHaiKangDVR::GetPlaceForPoint(WORD x, WORD y, WORD& lPlace)
{
	if ( y <= 32 )
	{
		if ( x <= 20 )
...
//OSD
bool VDHaiKangDVR::SetOSD(long lChannel,BOOL bShowOsd,WORD nOSDPlace,WORD nWordPlace,string& sWord)
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
...
bool VDHaiKangDVR::GetOSD(long lChannel,BOOL& bShowOsd,WORD& nOSDPlace,WORD& nWordPlace,string& sWord)
{	
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitPicCfg)
...
//帧率
bool VDHaiKangDVR::SetFrameRate(long lChannel,long nFrameRate, long nKeyFrameIntervals,bool bMainStream) 
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
...
bool VDHaiKangDVR::GetFrameRate(long lChannel,long& nFrameRate, long& nKeyFrameIntervals,bool bMainStream) 
{ 
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitCompressCfg)
...
//设置图像质量
bool VDHaiKangDVR::SetPictureQuality(long lChannel,long nLevel,bool bMainStream) 
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitCompressCfg)
...
//获取图像质量
bool VDHaiKangDVR::GetPictureQuality(long lChannel,long& nLevel,bool bMainStream) 
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitCompressCfg)
...
//最大码流
bool VDHaiKangDVR::SetMaxBPS(long lChannel,long nMaxBPS, bool vbr,bool bMainStream) 
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitCompressCfg)
...
bool VDHaiKangDVR::GetMaxBPS(long lChannel,long& nMaxBPS, bool& vbr,bool bMainStream) 
{ 
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitCompressCfg)
...
//图像格式
bool VDHaiKangDVR::SetPictureFormat(long lChannel,long nPicFormat, string& sSupportFormat,bool bMainStream) 
{ 
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitCompressCfg)
...
bool VDHaiKangDVR::GetPictureFormat(long lChannel,long& nPicFormat, string& sSupportFormat,bool bMainStream) 
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitCompressCfg)
...
//视频参数
bool VDHaiKangDVR::SetVideoPara(long lChannel,long nBrightness, long nContrast, long nSaturation /*饱和度*/, long nHue/*色调*/) 
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitPicCfg)
...
bool VDHaiKangDVR::GetVideoPara(long lChannel,long& nBrightness, long& nContrast, long& nSaturation /*饱和度*/, long& nHue/*色调*/) 
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitPicCfg)
...
//////////////////////////////////////////////////////////////////////////
//侦测区域
bool VDHaiKangDVR::SetDetectRect(long lChannel,vector<Rect>& Rects, BOOL bDectecting, long nDetectGrade, long nMaxRectNum) 
{ 	
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitPicCfg)
	...
	//m_VideoChannels[lChannel].m_dvr_pic_cfg.byOSDAttrib = 4;
	//m_VideoChannels[lChannel].m_dvr_pic_cfg.dwEnableHide = 0;
...
bool VDHaiKangDVR::GetDetectRect(long lChannel,vector<Rect>& Rects, BOOL& bDectecting, long& nDetectGrade, long& nMaxRectNum) 
{
	return false; 
}

//遮挡区域
bool VDHaiKangDVR::SetShadeRect(long lChannel,vector<Rect>& Rects, BOOL bLogoing, long nMaxRectNum) 
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitPicCfg)
...
bool VDHaiKangDVR::GetShadeRect(long lChannel,vector<Rect>& Rects, BOOL& bLogoing, long& nMaxRectNum) 
{
	lChannel+=1;
	if(lChannel>m_byChannelCount) lChannel+=(32-m_byChannelCount);
	if (!video_Para_records[lChannel].m_bInitPicCfg)
...
//开始语音呼叫
bool VDHaiKangDVR::StartAudioCall()
{
	StopAudioCall();

	//HxTrace("VDHaiKangDVR::StartAudioCall()");
...
//停止语音呼叫
bool VDHaiKangDVR::StopAudioCall()
{
	//if ( m_lAudioID != -1 )
	//{
	//	//HxTrace("VDHaiKangDVR::StopAudioCall()");
...
//转发音频数据到设备
bool VDHaiKangDVR::SendAudioData(void * pBuffer, long lSize)
{
...
//实现VD工厂
VDHaiKangDVRMaker::VDHaiKangDVRMaker()
{
}
...
VirtualDevice* VDHaiKangDVRMaker::Make()
{
	return new VDHaiKangDVR;
}

void VDHaiKangDVRMaker::Reclaim(VirtualDevice* vd)
{
	delete vd;
}
...
extern "C" __declspec(dllexport) void RegisterVDMaker(VirtualDeviceFactory* vd_factory)
{
	...
	vd_factory->AddVirtualDeviceMaker("HaiKangDVR", new VDHaiKangDVRMaker);
}