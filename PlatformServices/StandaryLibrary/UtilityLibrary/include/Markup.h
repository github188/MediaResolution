...
public:
	...
	{
		return Load( szFileName.c_str() );
	};
	bool SetDoc( const char* szDoc );
	bool SetDoc( std::string szDoc )
	{
		return SetDoc( szDoc.c_str() );
	}
	bool IsWellFormed();
	bool FindElem( const char* szName=NULL );
	bool FindElem( std::string szName )
	{
		return FindElem( szName.c_str() );
	}
	bool FindChildElem( const char* szName=NULL );
	bool FindChildElem( std::string szName )
	{
		return FindChildElem( szName.c_str() );
	}
	bool IntoElem();
	bool OutOfElem();
	...
	bool SavePos( std::string szPosName )
	{
		return SavePos(szPosName.c_str());
	}
	bool RestorePos( const char* szPosName="" );
	bool RestorePos( std::string szPosName )
	{
		return RestorePos(szPosName.c_str());
	}
	...
	bool Save( std::string szFileName )
	{
		return Save( szFileName.c_str() );
	}
	...
	bool AddElem( std::string szName, std::string szData=NULL, int nFlags=0 )
	{
		return AddElem( szName.c_str(), szData.c_str(), nFlags );
	}
	bool InsertElem( const char* szName, const char* szData=NULL, int nFlags=0 ) { return x_AddElem(szName,szData,nFlags|MNF_INSERT); };
	bool InsertElem( std::string szName, std::string szData, int nFlags=0 )
	{
		return InsertElem( szName.c_str(), szData.c_str(), nFlags );
	}
	bool AddChildElem( const char* szName, const char* szData=NULL, int nFlags=0 ) { return x_AddElem(szName,szData,nFlags|MNF_CHILD); };
	bool AddChildElem( std::string szName, std::string szData, int nFlags=0 )
	{
		return AddChildElem( szName.c_str(), szData.c_str(), nFlags );
	}
	...
	bool InsertChildElem( std::string szName, std::string szData, int nFlags=0 )
	{
		return InsertChildElem(szName.c_str(), szData.c_str(), nFlags);
	}
	bool AddElem( const char* szName, int nValue, int nFlags=0 ) { return x_AddElem(szName,nValue,nFlags); };
	bool AddElem( std::string szName, int nValue, int nFlags=0 )
	{
		return AddElem(szName.c_str(), nValue, nFlags );
	}
	bool InsertElem( const char* szName, int nValue, int nFlags=0 ) { return x_AddElem(szName,nValue,nFlags|MNF_INSERT); };
	bool InsertElem( std::string szName, int nValue, int nFlags=0 )
	{
		return InsertElem( szName.c_str(), nValue, nFlags );
	}
	bool AddChildElem( const char* szName, int nValue, int nFlags=0 ) { return x_AddElem(szName,nValue,nFlags|MNF_CHILD); };
	bool AddChildElem( std::string szName, int nValue, int nFlags=0 )
	{
		return AddChildElem( szName.c_str(), nValue, nFlags );
	}
	...
	bool AddAttrib( std::string szAttrib, std::string szValue )
	{
		return AddAttrib( szAttrib.c_str(), szValue.c_str() );
	}
	bool AddChildAttrib( const char* szAttrib, const char* szValue ) { return x_SetAttrib(m_iPosChild,szAttrib,szValue); };
	bool AddChildAttrib( std::string szAttrib, std::string szValue )
	{
		return AddChildAttrib(szAttrib.c_str(), szValue.c_str());
	}
	bool AddAttrib( const char* szAttrib, int nValue ) { return x_SetAttrib(m_iPos,szAttrib,nValue); };
	bool AddAttrib( std::string szAttrib, int nValue )
	{
		return AddAttrib(szAttrib.c_str(), nValue);
	}
	bool AddChildAttrib( const char* szAttrib, int nValue ) { return x_SetAttrib(m_iPosChild,szAttrib,nValue); };
	bool AddChildAttrib( std::string szAttrib, int nValue )
	{
		return AddChildAttrib(szAttrib.c_str(), nValue);
	}
	bool AddSubDoc( const char* szSubDoc ) { return x_AddSubDoc(szSubDoc,0); };
	bool AddSubDoc( std::string szSubDoc )
	{
		return AddSubDoc( szSubDoc.c_str() );
	}
	bool InsertSubDoc( const char* szSubDoc ) { return x_AddSubDoc(szSubDoc,MNF_INSERT); };
	bool InsertSubDoc( std::string szSubDoc )
	{
		return InsertSubDoc( szSubDoc.c_str() );
	}
	...
	bool AddChildSubDoc( std::string szSubDoc )
	{
		return AddChildSubDoc(szSubDoc.c_str());
	}
	bool InsertChildSubDoc( const char* szSubDoc ) { return x_AddSubDoc(szSubDoc,MNF_CHILD|MNF_INSERT); };
	bool InsertChildSubDoc( std::string szSubDoc )
	{
		return InsertChildSubDoc(szSubDoc.c_str());
	}
